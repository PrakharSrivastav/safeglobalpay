    ...
    public function create_vendor()
    {
        try
        {
            if ($this->session->userdata("is_logged_in") && ($this->session->userdata("v_role") == "0"))
            {
                ...
                $this->form_validation->set_rules("v_min_chargeback", "Min Chargeback Amount", "required|decimal");
                ...
                else
                {
                    ...
                    $vendor["v_min_chargeback"] = $this->sanitize($this->input->post("v_min_chargeback"));
    ...
    public function payout_vendor($vnd_id = null)
    {
        try
        {
            if ($this->session->userdata("is_logged_in") && ($this->session->userdata("v_role") == "0"))
            {
                ...
                $enddate = strtotime($this->input->post("enddate")) + 24 * 60 * 60;

                # get the details of the vendor by vendor-id
                $vendor = $this->vend->get_vendor($v_id);
                $percent = floatval($vendor[0]['v_percent']);

                # check if the start date is available in the form
                if (empty($startdate) || $startdate == false || is_null($startdate))
                {
                    $this->load->view("ayjoconsulting/admin/select-dates", array("vendor" => $vendor[0], "message" => "Please provide Start Date"));
                }
                # check if end date is provided in the form
                else if (empty($startdate) || $startdate == false || is_null($enddate))
                {
                    $this->load->view("ayjoconsulting/admin/select-dates", array("vendor" => $vendor[0], "message" => "Please provide End Date"));
                }
                # check if startdate < enddate
                else if ($enddate < $startdate)
                {
                    $this->load->view("ayjoconsulting/admin/select-dates", array("vendor" => $vendor[0], "message" => "Start Date is smaller than end Date"));
                }
                else
                {
                    $date = array("a.tr_timestamp>=" => $startdate, "a.tr_timestamp <= " => $enddate);

                    # get the list of all the transactions
                    $transactions = $this->trans->get_open_transactions($v_id, $date);
                    ...
                    if ($transactions !== false)
                    {
                        foreach ($transactions as $record)
                        {
                            ...
                            // print_r($record['tr_id']);echo "\n<br>";
                        }
                    }

                    // print_r($payment);
                    # if there is chargeback, then calculate chargeback penalty
                    if ($cbk_amt > 0.00)
                        $chargeback_penalty = $cbk_amt * floatval($vendor[0]['v_chargeback_penalty']) / 100.00;

                    # chargeback value will be  cargeback above or v_min_chargeback, whichever is bigger
                    if ($chargeback_penalty < floatval($vendor[0]['v_min_chargeback']))
                        $chargeback_penalty = floatval($vendor[0]['v_min_chargeback']);

                    # if there were any refunds, then calculate refund penatly
                    if ($ref_amt > 0.00)
    ...
    public function process_vendor_payout()
    {
        try
        {
            if ($this->session->userdata("is_logged_in") && ($this->session->userdata("v_role") == "0"))
            {
                ...
                $this->form_validation->set_rules("rr_amount", "Current Rolling Reserve", "required");
                $this->form_validation->set_rules("year", "Current Rolling Reserve", "required");
                $this->form_validation->set_rules("month", "Current Rolling Reserve", "required");
//
                if ($this->form_validation->run() === false)
                {
                ...
                else
                {
                    ...
                    {
                        if ($this->trans->perform_settlement($v_id, $starttime, $endtime))
                        {
                            $this->load->model("Vendor_model", "vendor");
                            ...
                            $data = array(
                                "v_id" => $v_id,
                                "rr_amount" => $this->input->post("rr_amount"),
                                "rr_month" => $month,
                                "rr_year" => $year,
                                "rr_status" => '0'
                            );
...
//                                    $current_rr_amount = $this->input->post("rr_amount");
...
//                 print_r($result);
    ...
    public function process_download()
    {
		...
		from_unixtime(a.tr_timestamp) as timestamp,
                a.Bank_Message as response_text,
                a.x_response_reason_text as response_details,
                c.c_name as customer_name,
                c.c_email as customer_email,
                c.c_contact as customer_phone
		FROM `sgp_transactions` `a` JOIN `sgp_vendors` `b` ON `a`.`tr_ref_id`=`b`.`v_id` 
                Join `sgp_customer` `c` ON `a`.`c_id` = `c`.`c_id`
                WHERE (`a`.`tr_ref_id` = '" . $v_id . "' ) ORDER BY `a`.`tr_timestamp` DESC";
                else if ($type === "2")
                    $sql = "SELECT 
		b.v_id as vendor_id, 
		b.v_name as name,
		...
		FROM `sgp_transactions` `a` JOIN `sgp_vendors` `b` ON `a`.`tr_ref_id`=`b`.`v_id` 
                Join `sgp_customer` `c` ON `a`.`c_id` = `c`.`c_id`
                WHERE (`a`.`tr_ref_id` = '" . $v_id . "') AND `a`.`tr_type` IN('0') ORDER BY `a`.`tr_timestamp` DESC";
                else if ($type === "1")
                    $sql = "SELECT 
		b.v_id as vendor_id, 
		b.v_name as name,
		...
		FROM `sgp_transactions` `a` JOIN `sgp_vendors` `b` ON `a`.`tr_ref_id`=`b`.`v_id` 
                Join `sgp_customer` `c` ON `a`.`c_id` = `c`.`c_id`
                WHERE (`a`.`tr_ref_id` = '" . $v_id . "' ) AND `a`.`tr_type` IN('3') ORDER BY `a`.`tr_timestamp` DESC";
                else if ($type === "0")
                    $sql = "SELECT 
		b.v_id as vendor_id, 
		b.v_name as name,
		...
		FROM `sgp_transactions` `a` JOIN `sgp_vendors` `b` ON `a`.`tr_ref_id`=`b`.`v_id` 
                Join `sgp_customer` `c` ON `a`.`c_id` = `c`.`c_id`
                WHERE (`a`.`tr_ref_id` = '" . $v_id . "' ) AND `a`.`tr_type` IN('1','2') ORDER BY `a`.`tr_timestamp` DESC";
...
//                print_r($where);
        ...
        try
        {
            if ($this->session->userdata("is_logged_in") && ($this->session->userdata("v_role") == "0"))
            {
                ...
                else
                {
                    ...
                    $tr = $this->trans->get_transaction($invoice);
                    $customer = $tr[0]['c_id'];
    ...
    public function select_report_dates()
    {
        $v_id = $this->uri->segment(3);
        $this->load->model("Vendor_model", "vendor");
        $vendor = $this->vendor->get_vendor($v_id);
//        print_r($vendor);
        ...
        {
            $this->load->view("ayjoconsulting/admin/select-dates", array("vendor" => $vendor[0]));
        }
    }
    ...
    public function get_invoice_details()
    {
        if ($this->input->is_ajax_request())
        {
            ...
            if (empty($invoice_id) || is_null($invoice_id) || $invoice_id == "")
            {
                die("fail");
            }
            if (empty($vendor_id) || is_null($vendor_id) || $vendor_id == "")
            {
                die("fail");
            }
            $invoice = $this->trans->get_transaction($invoice_id);
            if ($invoice && !empty($invoice) && count($invoice) > 0)
            {
                if ($invoice[0]['tr_ref_id'] == $vendor_id)
                {
                    print_r($invoice[0]['tr_amount']);
                }
                else
                {
                    die("fail");
